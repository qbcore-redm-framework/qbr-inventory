...
function handleDragDrop() {
  ...
  $('#item-use').droppable({
    ...
    drop: function (event, ui) {
      ...
      if (fromData.useable && fromData.type !== 'weapon') {
      ...
      } else if (fromData.type === 'weapon') {
        if (fromData.shoudClose) {
          Inventory.Close();
        }

        $.post(
          `https://qbr-inventory/UseWeaponItem`,
          JSON.stringify({
            inventory: fromInventory,
            item: fromData,
          })
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
  ...
  if (fromData !== undefined && fromData.amount >= $toAmount) {
    ...
    if (
      ...
      newData.attachPoint = toData.attachPoint || false;
      ...
      if (fromData.amount == $toAmount) {
        ...
        newDataFrom.attachPoint = fromData.attachPoint || false;
      ...
      if (fromData.amount == $toAmount) {
        ...
        newDataTo.attachPoint = fromData.attachPoint || false;
  ...
  window.onload = function (e) {
    window.addEventListener('message', function (event) {
      switch (event.data.action) {
        ...
        case 'RobMoney':
          $('.inv-options-list').append(
            '<div class="inv-option-item" id="rob-money"><p>NEEM GELD</p></div>'
          );
